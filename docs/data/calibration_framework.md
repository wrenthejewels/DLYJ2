...
- `occupation_industry_mix.csv` is now emitted from the same ACS pass as preparation for the next BTOS adoption-context integration
...
1. `BTOS` next
2. `O*NET 30.2` refresh after the stronger calibration layers are stable
...
- `industry_ai_adoption_context.csv`
  ...
  - intended join path: `industry_ai_adoption_context.csv` -> `occupation_industry_mix.csv` -> occupation-level adoption-context target
